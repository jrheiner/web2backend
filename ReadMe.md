...
file is in the correct directory, run eslint, and run mocha tests for the webservices.
...
```
    checkConfig         - Check if config can be found in correct directory
...
If the problem still persists the manual step is to build the Angular project by using the 
...
functionality in the web control panel but its *free*. 
...
25 GB of net viewing bandwidth on a monthly basis. The reason as why these two cloud providers were chooses 
are simply because they fulfilled the technical requirements with permanent free tier, with limitations that
only have small or no impact the project.

Adding **user accounts** brought some challenges with regard to security and session management. For example some
...
user. During a session the server auth token is stored in session storage, however tampering with it will invalidate, 
...
technology was a bit challenging at times but exiting for sure, and I think it tackles the challenges mentioned earlier.

To further try to implement security standards with this project, user passwords are obviously not stored in plain text,
